...
analyzer = BehaviorMetricsAnalyzer()
...
async def get_profiles_by_segment(
    segment_tag: str, tenant_id: str = Query("default"), db: Session = Depends(get_db)
):
    """根据分群标签获取用户列表"""
...
async def list_segments(
    tenant_id: str = Query("default"), db: Session = Depends(get_db)
):
    """列出所有分群标签"""
    profiles = db.query(UserProfile).filter(UserProfile.tenant_id == tenant_id).all()

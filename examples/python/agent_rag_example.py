...
"""
Agent 平台 RAG 应用示例
演示如何整合 Data + Model + Agent 平台实现知识问答
"""
...
class RAGApplication:
    ...
    def health_check(self) -> Dict:
        ...
        try:
            ...
            status["data"] = "OK" if r.status_code == 200 else "ERROR"
        ...
        try:
            ...
            status["model"] = "OK" if r.status_code == 200 else "ERROR"
        except Exception as e:
        ...
        try:
            ...
            status["agent"] = "OK" if r.status_code == 200 else "ERROR"
        except Exception as e:
            status["agent"] = f"ERROR: {e}"
    ...
    def sql_query(self, question: str) -> Dict:
        ...
        response = self.session.get(
            f"{self.data_url}/api/v1/metadata/databases/sales_dw/tables/orders"
        )
...
请只返回 SQL 查询语句，不要其他内容。"""
    ...
    def list_datasets(self) -> List[Dict]:
        """获取可用数据集列表"""
        response = self.session.get(f"{self.data_url}/api/v1/datasets")
        response.raise_for_status()
        ...
        self.documents = [
            {
                ...
                1. Data 平台 - 数据治理与开发平台，负责数据的采集、清洗、治理和存储
                2. Model 平台 - 云原生 MLOps 平台，提供模型训练、部署和服务化能力
                3. Agent 平台 - 大模型应用开发平台，支持 RAG、Agent 等应用编排

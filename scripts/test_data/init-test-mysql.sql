...
-- 端口：3316
-- 数据库名称：test_ecommerce, test_user_mgmt, test_logs
--
-- =============================================================================
...
BEGIN
    ...
    WHILE i <= count DO
        ...
        VALUES (username, email, 'hashed_password', CONCAT('用户', i),
...
BEGIN
    ...
    WHILE i <= count DO
        SET v_order_id = FLOOR(1 + RAND() * 2000);
        SET v_product_id = FLOOR(1 + RAND() * 500);
        SET v_quantity = FLOOR(1 + RAND() * 5);

        -- 获取商品信息
        SELECT product_name, product_code, price INTO v_product_name, v_product_code, v_unit_price
        FROM products WHERE id = v_product_id LIMIT 1;
        ...
        VALUES (v_order_id, v_product_id, v_product_name, v_product_code, v_unit_price, v_quantity, v_subtotal);
...
BEGIN
    ...
    WHILE i <= count DO
        SET v_user_id = FLOOR(1 + RAND() * 500);
        SET v_username = CONCAT('user_', LPAD(v_user_id, 5, '0'));

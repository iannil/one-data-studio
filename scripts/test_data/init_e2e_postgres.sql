-- =============================================================================
-- ONE-DATA-STUDIO E2E 测试数据初始化脚本 (PostgreSQL)
-- =============================================================================
--
...
--
-- 使用方法：
--   PGPASSWORD=e2epostgres123 psql -h localhost -p 5438 -U postgres -d e2e_ecommerce_pg -f init_e2e_postgres.sql
--
-- =============================================================================
...
BEGIN
    WHILE i <= count LOOP
        username := 'e2e_pg_user_' || LPAD(i::TEXT, 5, '0');
        email := username || '@e2e.pg.example.com';
...
BEGIN
    WHILE i <= count LOOP
        order_no := 'E2E_PG_ORD' || TO_CHAR(CURRENT_TIMESTAMP, 'YYYYMMDDHH24MISS') || LPAD(i::TEXT, 4, '0');
...
SELECT generate_orders(1500);
SELECT generate_order_items(3000);

-- 显示数据统计
DO $$
BEGIN
    RAISE NOTICE 'E2E PostgreSQL Test data initialization completed!';
END $$;

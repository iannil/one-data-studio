...
**原因**: 文档统计停留在 2025-01-24，此后新增了 admin-api、model-api、ocr-service、behavior-service 等多个服务。

### 3.2 服务发现差异

旧文档仅记录了 4 个后端服务（data-api、agent-api、openai-proxy、shared），实际已有 8 个服务模块（新增 admin-api、model-api、ocr-service、behavior-service）。
...
| `services/data-api/app.py` | 从实际表中获取样本数据 |
...
2. **为新增服务补充文档**: admin-api、model-api、ocr-service 需要 API 端点文档

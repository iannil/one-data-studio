...
test.describe('数据管理员完整流程', () => {
  ...
  test.describe('DM-DS: 数据源管理', () => {
    test('DM-DS-001: 注册 MySQL 数据源', async ({ request }) => {
      const response = await request.post(`${API_BASE}/api/v1/datasources`, {
    ...
    test('DM-DS-004: 连接测试 - 失败场景', async ({ request }) => {
      const response = await request.post(`${API_BASE}/api/v1/datasources/test`, {
        headers: { Authorization: `Bearer ${adminToken}` },
        data: {
          type: 'mysql',
          host: 'invalid-host',
    ...
    test('DM-DS-005: 编辑数据源配置', async ({ request }) => {
      // 先创建数据源
      const createResp = await request.post(`${API_BASE}/api/v1/datasources`, {
        headers: { Authorization: `Bearer ${adminToken}` },
        data: {
      ...
      // 编辑数据源
      const editResp = await request.put(`${API_BASE}/api/v1/datasources/${dsId}`, {
        headers: { Authorization: `Bearer ${adminToken}` },
        data: {
    ...
    test('DM-DS-006: 删除未引用的数据源', async ({ request }) => {
      ...
      // 删除数据源
      const deleteResp = await request.delete(`${API_BASE}/api/v1/datasources/${dsId}`, {
        headers: { Authorization: `Bearer ${adminToken}` },
      });
  ...
  test.describe('DM-MS: 元数据自动扫描', () => {
    test('DM-MS-001: 触发元数据扫描', async ({ request }) => {
      const response = await request.post(`${API_BASE}/api/v1/metadata/scan`, {
        headers: { Authorization: `Bearer ${adminToken}` },
        data: {
    ...
    test('DM-MS-002: 扫描结果包含表和列信息', async ({ request }) => {
      // 触发扫描
      const scanResp = await request.post(`${API_BASE}/api/v1/metadata/scan`, {
        headers: { Authorization: `Bearer ${adminToken}` },
        data: {
          datasource_id: 'test-datasource-1',
          scan_type: 'full',
      ...
      // 查询扫描结果
      const resultResp = await request.get(`${API_BASE}/api/v1/metadata/scan/${taskId}/result`, {
        headers: { Authorization: `Bearer ${adminToken}` },
      });
    ...
    test('DM-MS-003: AI 自动标注', async ({ request }) => {
      const response = await request.post(`${API_BASE}/api/v1/metadata/ai-annotate`, {
        headers: { Authorization: `Bearer ${adminToken}` },
        data: {
          datasource_id: 'test-datasource-1',
  ...
  test.describe('DM-SD: 敏感数据识别', () => {
    test('DM-SD-001: 触发敏感数据扫描', async ({ request }) => {
      const response = await request.post(`${API_BASE}/api/v1/sensitivity/scan`, {
        headers: { Authorization: `Bearer ${adminToken}` },
        data: {
          datasource_id: 'test-datasource-1',
          scan_scope: 'all',
    ...
    test('DM-SD-010: 自动生成脱敏规则', async ({ request }) => {
      const response = await request.post(`${API_BASE}/api/v1/masking/rules/auto-generate`, {
        headers: { Authorization: `Bearer ${adminToken}` },
        data: {
          datasource_id: 'test-datasource-1',
          table_name: 'test_users',
  ...
  test.describe('DM-AS: 资产编目与价值评估', () => {
    test('DM-AS-001: 创建资产目录', async ({ request }) => {
      const response = await request.post(`${API_BASE}/api/v1/assets/catalog`, {
        headers: { Authorization: `Bearer ${adminToken}` },
        data: {
    ...
    test('DM-AS-002: 资产价值评估', async ({ request }) => {
      const response = await request.post(`${API_BASE}/api/v1/assets/evaluate`, {
        headers: { Authorization: `Bearer ${adminToken}` },
        data: {
    ...
    test('DM-AS-007: 资产搜索', async ({ request }) => {
      const response = await request.get(`${API_BASE}/api/v1/assets/search?q=test`, {
        headers: { Authorization: `Bearer ${adminToken}` },
      });
  ...
  test.describe('DM-SY: 元数据同步与血缘', () => {
    test('DM-SY-001: 触发元数据同步', async ({ request }) => {
      const response = await request.post(`${API_BASE}/api/v1/lineage/sync`, {
        headers: { Authorization: `Bearer ${adminToken}` },
        data: {
          datasource_id: 'test-datasource-1',
          sync_type: 'full',
    ...
    test('DM-SY-006: 查询数据血缘', async ({ request }) => {
      const response = await request.get(
        `${API_BASE}/api/v1/lineage/graph?table=test_users&depth=3`,
        { headers: { Authorization: `Bearer ${adminToken}` } }
      );
  ...
  test.describe('DM-PM: 权限与安全管理', () => {
    test('DM-PM-001: 查看角色列表', async ({ request }) => {
      const response = await request.get(`${API_BASE}/api/v1/auth/roles`, {
        headers: { Authorization: `Bearer ${adminToken}` },
      });
    ...
    test('DM-PM-003: 分配数据权限', async ({ request }) => {
      const response = await request.post(`${API_BASE}/api/v1/auth/permissions`, {
        headers: { Authorization: `Bearer ${adminToken}` },
        data: {
          user_id: 'test-user-1',
          resource_type: 'datasource',
          resource_id: 'test-datasource-1',
          permission: 'read',
        },
      });
    ...
    test('DM-PM-004: 验证权限生效', async ({ request }) => {
      // 先分配权限
      await request.post(`${API_BASE}/api/v1/auth/permissions`, {
        headers: { Authorization: `Bearer ${adminToken}` },
        data: {
          user_id: 'test-user-1',
          resource_type: 'datasource',
      ...
      const checkResp = await request.get(
        `${API_BASE}/api/v1/auth/permissions/check?user_id=test-user-1&resource_type=datasource&resource_id=test-datasource-1&action=read`,
        { headers: { Authorization: `Bearer ${adminToken}` } }
      );
